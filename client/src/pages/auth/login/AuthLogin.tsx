...
const loginSchema = z.object({
  email: z
    ...
    .max(36, 'El email no puede exceder de 32 caracteres')
    .email('El email es invalido'),
  password: z
    .string()
    .min(8, 'El password debe tener al menos 8 caracteres')
    .max(36, 'El password no puede exceder de 32 caracteres'),
});
...
const AuthLogin = () => {
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!validate.success) {
      ...
      setError({
        email: formatedError.email?._errors[0],
        password: formatedError.password?._errors[0],
      });
      return;
    }
  ...
  return <FormUI error={error} handleSubmit={handleSubmit} />;
};
